...
class GlowText {
 ...
 public:
    ...
    GlowText(const Font& font, const String& text, int32 blur,
             double gamma = 2.0)
        : m_font(font), m_text(text), m_offset(blur + 4, blur + 4) {
    ...
    const Texture& getGlowTexture() const { return m_texture; }

    RectF draw(const Vec2& pos, const Color& glow = Palette::White,
               const Color& text = Palette::White) const {
        m_texture.draw(pos, glow);

        return m_font(m_text).draw(pos + m_offset, text);
    }

    RectF drawCenter(double y, const Color& glow = Palette::White,
                     const Color& text = Palette::White) const {
        return drawCenter(Vec2(Window::Width() / 2, y), glow, text);
    }

    RectF drawCenter(const Vec2& pos, const Color& glow = Palette::White,
                     const Color& text = Palette::White) const {
